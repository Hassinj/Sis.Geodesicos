...
def SIRGAS(lon, lat, h):
    ...
    Y = (p) * math.sin(lon)
    Z = ((N * (1 - (e * e))) + h) * (math.sin(lat))
    print("\nb:", b, "\nc:", c, "\ne:", e, "\ne':", el, "\nN:", N, "\np:", p, "\n -> X_SIRGAS2000:", X, " Y_SIRGAS2000:", Y, " Z_SIRGAS2000:", Z)
    return(X, Y, Z)
...
def CORREGO(lon, lat, h):
    ...
    Y = (p) * math.sin(lon)
    Z = ((N * (1 - (e * e))) + h) * (math.sin(lat))
    print("\nb:", b, "\nc:", c, "\ne:", e, "\ne':", el, "\nN:", N, "\np:", p, "\n -> X_CORREGOALEGRE:", X, " Y_CORREGOALEGRE:", Y, " Z_CORREGOALEGRE:", Z)
    return (X, Y, Z)


def paraSAD_69(X, Y, Z):
    print(" -> X_SAD69:", X, "Y_SAD69:", Y, "Z_SAD69:", Z)
    a = 6378160.0
    f = 1 / 298.25
    b = (a * (1 - f))
    c = math.sqrt((a * a) - (b * b))
    ...
    print("\nb:", b, "\nc:", c, "\ne:", e, "\ne':", el, "\nN:", N, "\np:", p, "\nTheta:", theta, "\n Latitude: ", glat1,"° ", abs(minlat1), "' ", abs(seglat1),"'' ",  " \n Longitude: ", glon1,"° ", abs(minlon1), "' ", abs(seglon1),"'' ", "\n Altura: ", h1, "m")
    return (lat1, lon1, h1)
    
def paraWGS84(X, Y, Z):
    print(" -> X_WGS84:", X, "Y_WGS84:", Y, "Z_WGS84:", Z)
    a = 6378137.0
    f = 0.00335281066
    b = (a * (1 - f))
    c = math.sqrt((a * a) - (b * b))
...
def paraSIRGAS(X, Y, Z):
    print(" -> X_SIRGAS2000:", X, "Y_SIRGAS2000:", Y, "Z_SIRGAS2000:", Z)
    a = 6378137.0
    f = 0.00335281068
    b = (a * (1 - f))
    c = math.sqrt((a * a) - (b * b))
...
def paraCORREGO(X, Y, Z):
    print(" -> X_CORREGOALEGRE:", X, "Y_CORREGOALEGRE:", Y, "Z_CORREGOALEGRE:", Z)
    a = 6378388.0
    f = 0.00336700337
    b = (a * (1 - f))
    c = math.sqrt((a * a) - (b * b))
